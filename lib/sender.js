...
Sender.prototype.send = function(message, recipient, options, callback) {
    ...
    if(message.params.data && message.params.data.from) {
    ...
    getRequestBody(message, recipient, function(err, body) {
        ...
        request(request_options, function (err, res, resBodyJSON) {
            ...
            if (res.statusCode !== 200) {
                ...
                resBodyJSON = {
                  success: 0,
                  failure: Array.isArray(body.registration_ids) ? body.registration_ids.length : 0,
                  canonical_ids: 0,
                  results: [{
                    error: `Invalid request (${res.statusCode}): ${resBodyJSON}`
                  }]
                };
            }
